...
- (void)viewDidLoad
{
    ...
    self.baseDir = [NSString stringWithFormat:@"%@/",  [docFolders lastObject]];
    if (![[NSFileManager defaultManager] fileExistsAtPath:self.baseDir])
    {
        NSError* err = nil;
        [[NSFileManager defaultManager] createDirectoryAtPath:self.baseDir withIntermediateDirectories:YES attributes:nil error:&err];
    }
    //加载参数 是否匿名、用户名、密码、端口
    [self loadSetting];
...
-(void)onFtpParasChanged:(NSNotification*)info
{
    ...
    if (needStopServer)
    {
        ...
        {
            serverStopedByUser_ = true;
            [[gaeaToastView getInstance] showToastViewWithContent:@"远程管理设置信息已修改，请重新启动" inViewController:self];
        }
        else
        {
            [[gaeaToastView getInstance] showToastViewWithContent:@"修改成功" inViewController:self];
        }
        [self stopFtpServer];
        currentServerState_ = SERVER_STATE_STOP;
        [self setAllStateViewsState];
