//
//  empDownLoadManager.m
//  ExMobi
//
//  Created by  hyc on 13-3-27.
//
//
...
@implementation gaeaToastView

static gaeaToastView *instance = nil;

+ (gaeaToastView *)getInstance {
    if (instance == nil)
    {
        instance = [[gaeaToastView alloc] init];
    }
	return instance;
}
...
-(void)showToastViewWithContent:(NSString*)content inViewController:(UIViewController*)inViewController
{
    if (!inViewController)
    {
        return;
    ...
    [inViewController.view addSubview:toastView_];
    ...
    toastView_.center = CGPointMake(inViewController.view.center.x, 416);
...
{
    UIViewController* topViewController = [self getTopViewController];
    [self showToastViewWithContent:content inViewController:topViewController];
}
