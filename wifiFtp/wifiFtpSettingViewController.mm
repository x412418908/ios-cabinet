...
-(void)rightBarButtonItemDown
{
    ...
    if (!self.isAnonymous)
    {
        ...
        {
            [[gaeaToastView getInstance] showToastViewWithContent:@"用户名不能为空！" inViewController:self];
            return;
        }
        ...
        {
            [[gaeaToastView getInstance] showToastViewWithContent:@"密码不能为空！" inViewController:self];
            return;
        }
    ...
    {
        [[gaeaToastView getInstance] showToastViewWithContent:@"端口不能为空！" inViewController:self];
        return;
    }
    //保存设置
    [self saveSetting];
...
-(void)loadSetting
{
    ...
    NSString* plistPath = [NSString stringWithFormat:@"%@/ExMobi/data/sys/ftp.plist",  [docFolders lastObject]];
...
{
    NSArray *docFolders = NSSearchPathForDirectoriesInDomains (NSDocumentDirectory, NSUserDomainMask, YES );
    NSString* plistParentPath = [NSString stringWithFormat:@"%@/ExMobi/data/sys/",  [docFolders lastObject]];
    NSString* plistPath = [NSString stringWithFormat:@"%@/ExMobi/data/sys/ftp.plist",  [docFolders lastObject]];
    if (![[NSFileManager defaultManager] fileExistsAtPath:plistPath])
    {
        NSURL* url = [NSURL fileURLWithPath:plistParentPath];
        NSError* err = nil;
